...
# =========================================================================
# AUR APP PASSWORD DAALEIN
# =========================================================================
GMAIL_ID = "add a sending Email hear"
GMAIL_APP_PASSWORD = "gaail app key to sending email"
...
def get_my_team(username, role):
    ...
    if role == 'admin':
        rows = conn.execute("SELECT username FROM users WHERE username != ? AND status = 'approved'", (username,)).fetchall()
        team = [r['username'] for r in rows]
    elif role == 'hr':
        managers = conn.execute("SELECT username FROM users WHERE role = 'manager' AND reports_to = ? AND status = 'approved'", (username,)).fetchall()
        ...
        if manager_usernames:
            ...
            emps = conn.execute(f"SELECT username FROM users WHERE role = 'employee' AND reports_to IN ({placeholders}) AND status = 'approved'", manager_usernames).fetchall()
            team.extend([e['username'] for e in emps])
    elif role == 'manager':
        emps = conn.execute("SELECT username FROM users WHERE role = 'employee' AND reports_to = ? AND status = 'approved'", (username,)).fetchall()
...
# --- AUTH & INVITE ROUTES (Fixed Approvals & Signup) ---

@app.route('/signup', methods=['GET', 'POST'])
def signup():
    if request.method == 'POST':
        u = request.form['username'].strip().replace(" ", "_")
        p = request.form['password']
        r = request.form.get('role', 'employee').lower()
        conn = get_db_connection()
        try:
            # User will be created with status 'pending'
            conn.execute("INSERT INTO users (username, password, role, status) VALUES (?, ?, ?, 'pending')", (u, p, r))
            conn.commit()
            conn.close()
...
def approvals():
    ...
    pending_users = conn.execute("SELECT username, role FROM users WHERE status = 'pending'").fetchall()
    # Fetch managers/admins to assign reports_to
    managers = conn.execute("SELECT username, role FROM users WHERE role IN ('admin', 'manager', 'hr') AND status = 'approved'").fetchall()
    conn.close()
    return render_template('approvals.html', users=pending_users, managers_list=managers)


@app.route('/approve_user/<username>', methods=['POST'])
@role_required('admin', 'hr')
def approve_user(username):
    reports_to = request.form.get('reports_to', 'admin')
    conn = get_db_connection()
    conn.execute("UPDATE users SET status = 'approved', reports_to = ? WHERE username = ?", (reports_to, username))
    conn.commit()
    conn.close()
...
def reject_user(username):
    conn = get_db_connection()
    conn.execute("DELETE FROM users WHERE username = ? AND status = 'pending'", (username,))
    conn.commit()
    conn.close()
    return redirect(url_for('approvals'))
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            if user_status != 'approved': 
                return render_template('login.html', error="Your account is not approved.")
